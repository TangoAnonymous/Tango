    ...
    while True:
        try:
            ...
            for task in result_list:
                ...
                dockerInfo_send_dict[service_id] = {
...
def decision_get_make(
    decision_get_make_queue,
):
...
def changeResCg(
    ...
    if path_array:
        writeRes(
            path_array, change_num, isNumberAdd, specRes_child, serviceID, lock_array
        )
    else:
...
def THR_lc_work_do(pool_args):
    ...
    record_path_for_release_1 = changeResCg(
        "cpu,cpuacct",
        data[1],
        "cpu.cfs_quota_us",
        int(data[2][0]),
    ...
    record_path_for_release_2 = changeResCg(
        "cpu,cpuacct",
        data[1],
        "cpu.shares",
        int(data[2][1]),
...
def per_docker_watchdog(pool_args):
    ...
    while True:
        try:
            ...
            cpu_percent = (
                ((utime_2 - utime_1) + (stime_2 - stime_1)) * 100.0 / (100 * 0.1 * 1)
            )
...
def docker_quick_info():
    ...
    threading.Thread(
        target=(container_api_search),
        args=(container_check_lock, container_api_search_dict),
    ).start()
    pool = ThreadPoolExecutor()
                ...
                for service_id in container_api_search_dict:
                    if service_id not in container_watch_dict:
                        task = pool.submit(
                            ...
                            (
                                dockerInfo_send_dict,
                                container_api_search_dict[service_id],
                                service_id,
                                container_watch_dict,
                            ),
                        )
                        container_watch_dict[service_id] = task
                        task.add_done_callback(thread_pool_callback)
...
if __name__ == "__main__":
    ...
    for i in range(2):
        process_LC_work_do = multiprocessing.Process(
            target=lc_work_do, args=(lc_queue, lock_array)
        )
